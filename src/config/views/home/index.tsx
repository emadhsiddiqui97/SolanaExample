...
import AirdropMultiple from "components/AirDropMultiple/AirdropMultiple";
...
export const HomeView: FC = ({}) => {
  ...
  const tokenBalance = useUserTokenBalanceStore((s) => s.tokenBalance);
  ...
  useEffect(() => {
    if (wallet.publicKey) {
      ...
      getUserTokenBalance(wallet.publicKey);
    }
      ...
      <div className="md:hero-content flex flex-col">
        ...
        <div className="flex flex-col mt-2">
          ...
          <h4 className="md:w-full text-2xl text-slate-300 my-2">
            {wallet && (
              <div className="flex flex-row justify-center">
                <div>{(tokenBalance || 0).toLocaleString()}</div>
                <div className="text-slate-600 ml-2">Tokens</div>
              </div>
            )}
          </h4>
        </div>
