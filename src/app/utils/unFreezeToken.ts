...
export const unFreezeToken = async (wallet: AnchorWallet) => {
  ...
  try {
    ...
    signedTx
      ...
      : null;
    console.log(
      `unfroze Token: https://explorer.solana.com/tx/${confirmedTx}?cluster=devnet`
    );
  } catch (error: any) {
    if (error.message && error.message.includes("User rejected the request")) {
      console.log("transaction cancelled");
    } else {
      console.error("Failed to unfreeze token: ", error.message);
    }
  }
};
